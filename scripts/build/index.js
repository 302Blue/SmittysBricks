...
function fillBoard() {
    for (let j = 0; j < 8; j++) {
        for (let i = 0; i < 10; i++) {
            let x = (i * BRICK_WIDTH) + (BRICK_WIDTH / 2);
            let y = BOARD_HEIGHT - (j * BRICK_HEIGHT) - (BRICK_HEIGHT / 2);
            let r = Math.ceil(Math.random() * 25);
            if (r > 3) {
                r = 0;
            }
...
function initCustomLevelMode() {
    ...
    for (var i = 0; i < localStorage.length; i++) {
        ...
        $("#level-editor-open-list").append('<button type="button" class="list-group-item list-group-item-action">'
            + levelName
            + '</button>');
    }
    ...
    $("#level-editor-open-confirm").on("click", function (evt) {
        if (typeof (Storage) !== "undefined") {
            ...
            if (bricksJSON == null) {
                ...
                let bricksArray = JSON.parse(bricksJSON);
...
let ghostBrick;
let ghostPowerup;
function placeGhostBrick(screenX, screenY, ghostBrick) {
    // Calculate board scaling and offset
    let scale = Math.min(window.innerWidth / BOARD_WIDTH, (window.innerHeight - HEADER_HEIGHT) / BOARD_HEIGHT);
...
function update(delta) {
    if (gameStart && gameState.getGameMode() != "LevelEditor") {
        if (gameState.getGameMode() == 'AILab') {
            ...
            let predict = ai.predict(paddle, ball, board);
